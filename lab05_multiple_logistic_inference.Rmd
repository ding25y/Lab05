...
summary(logistic_fit_2$finalModel)
...
```{r}
anova(logistic_fit_2$finalModel,logistic_fit$finalModel,test="Chisq")
```
The p value is bigger than 0.05, thus we cannot reject the null hypothesis that the `claim_location` variable is not important for predicting the judge's decision.
...
summary(logistic_fit_3$finalModel)
anova(logistic_fit_3$finalModel,logistic_fit_2$finalModel,test="Chisq")
```
There are statistically significant differences in the chances of granting an appeal for different judges because the p value is 3.12e-07, which indciates that the two model differs. 
...
If logit_success increases by one unit, the odds of judge decision succession rate will increase by  1.3005.
...
NO, because the variable judge Iacobucci has an estimate of  -2.56776 (significant), which means that if the case is heard by Judge Iacobucci, the odds of judge decision succession rate will decrease by 2.56776, which is not what I want. 